...
const nannyAuth = require("../models/NannyAuth");
...
route.post("/", upload.array("documents"), async (req, res) => {
  try {
    ...
    const findNanny = await nannyAuth.findOne({ email: requestData.email });
    if (!findNanny) {
      const newPassword = await bcrypt.hash(requestData.password, 10);
      if (newPassword) {
        requestData.password = newPassword;
        const data = await nannyAuth.create(requestData);
        if (data) {
...
route.post("/login", async (req, res) => {
  try {
    const findNanny = await nannyAuth.findOne({ email: req.body.email });
    if (findNanny) {
